# -*- coding: utf-8 -*-
"""
Created on Mon May  6 10:25:10 2024
...
prop_sim["energy"]=653#incident photon energies in eV
prop_sim["angles"]=np.array([6,7.5,10,13,15,20,25,30,35,40])
...
if prop_sim["type"]=="Crystal":
    prop_sim["f_bulk"]=amorph_LSMO._atom.get_atomic_form_factor(prop_sim["energy"])-complex(0,1)*12
...
prop_sim["phi_rotations"]=np.linspace(0,90,2)#phi rotation in degrees
...
prop_3D['Mx']="../Magnetization Files/Mx_Analytic_crossover=5.csv"#micromagnetic simulation inputs
prop_3D['My']="../Magnetization Files/My_Analytic_crossover=5.csv"#don´t forget to put these files in the working directory, or add the path to the filename
prop_3D['Mz']="../Magnetization Files/Mz_Analytic_crossover=5.csv"
...
prop_3D["shape"]=[2,384,80]#array size of the micromagnetic input
...
prop_3D["z_pixel_sizes"]=0.388*3
prop_3D["total_thickness"]=0.388*3*prop_3D["shape"][2]*1e-9
...
for k in range(output1.shape[0]):
    ...
    # plt.plot(((np.abs(Intensity1[128,:]*beam_stop[128,:]))+(np.abs(Intensity2[128,:]*beam_stop[128,:]))))
    # plt.show()
    # plt.plot(((np.abs(Intensity1[128,:]*beam_stop[128,:]))-(np.abs(Intensity2[128,:]*beam_stop[128,:]))))
    # plt.show()
   
    # specular1.append(Intensity1[128,128])
    # specular2.append(Intensity2[128,128])
    temp1=output2[k,:,:,:]
    temp2=output1[k,:,:,:]
    Intensity1=(In1[0]*temp1[:,0,0]+In1[1]*temp1[:,0,1])*np.conj(In1[0]*temp1[:,0,0]+In1[1]*temp1[:,0,1])+\
    (In1[0]*temp1[:,1,0]+In1[1]*temp1[:,1,1])*np.conj(In1[0]*temp1[:,1,0]+In1[1]*temp1[:,1,1])
    ...
    Intensities_diff[k,:]=((np.abs(Intensity1*beam_stop))-(np.abs(Intensity2*beam_stop)))
...
#ani = animation.ArtistAnimation(fig2, ims, interval=50)
    #animation = FuncAnimation(fig2, animate,frames=300,interval=100)
#ani.save(f"animation_PCA_validation_{image}.gif")
#ani.save("testanimation.gif",writer="pillow")
