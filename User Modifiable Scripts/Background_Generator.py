...
prop_sim['det_dx']=13.5e-6*8#detector pixel size in metres
...
if prop_sim["f_manual_input"]==True:
    prop_sim["f_charge_manual"]=complex(-30.,15.)#these manual inputs can be later replaced with an energy dependent function interpolating an externally supplied text file
    prop_sim["f_mag_manual"]=complex(5.,5.)
angles_outer=np.linspace(14,18,41)
for jj in range(len(angles_outer)):
    prop_sim["angles"]=[angles_outer[jj]]#incident angles in degrees
    ...
    bkg_params["eta_par"] = np.array([7.0e-8,7.0e-8])#in plane correlation lengths (charge and magnetic)
    bkg_params["h"] = np.array([0.15,0.15])#roughness exponents (charge and magnetic)  
    ...
    for k in range(output1.shape[0]):
        ...
        np.savetxt("../Magnetization_Files/Intensity_reduced_absorption1"+str(jj)+".csv", Intensity1_flattened, delimiter=",")
        np.savetxt("../Magnetization_Files/Intensity_reduced_absorption2"+str(jj)+".csv", Intensity2_flattened, delimiter=",")
        ...
        np.savetxt("../Magnetization_Files/IB_reduced_absorption1"+str(jj)+".csv", Intensity1_flattened, delimiter=",")
        np.savetxt("../Magnetization_Files/IB_reduced_absorption2"+str(jj)+".csv", Intensity2_flattened, delimiter=",")
